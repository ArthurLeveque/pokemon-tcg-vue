<template>
  <div class="inputs">
    <div class="filters">
      ...
      <select name="types" id="types" @change="filterDeckModified">
        <option value="null">Type</option>
        <option v-for="(type, index) in types.data" :key="index" :value="type" v-bind:selected="filters.types == type">{{ type }}</option>
      </select>

      <select name="rarities" id="rarities" @change="filterDeckModified">
      ...
      <select name="sets" id="sets" @change="filterDeckModified">
    ...
    <div class="search">
      <input type="text" name="name" id="name" placeholder="Search card by name..." @input="filterDeckModified" :value="filters.name">
    </div>
  </div>
</template>
...
export default {
  ...
  methods:{
    ...mapActions('cards', ['getTypes', 'getRarities', 'getSets', 'filterDeckModified']),
  }
}
</script>
